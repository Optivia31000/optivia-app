...
# --- INTERFACE ---
st.set_page_config(page_title="CALCULATEUR OPTIVIA", layout="centered")
...
with col1:
    src = st.selectbox("DÉPART", list(DEPTS.keys()), format_func=lambda x: f"{str(x).zfill(2)} - {DEPTS[x]}", index=29)
with col2:
    dst = st.selectbox("ARRIVÉE", list(DEPTS.keys()), format_func=lambda x: f"{str(x).zfill(2)} - {DEPTS[x]}", index=72)

st.subheader("👇 SAISIR LES KM ICI")
dist_auto = abs(src - (20 if isinstance(dst, str) else dst)) * 8 + 150
km_real = st.number_input("Distance Réelle (km)", value=km_auto if 'km_auto' in locals() else dist_auto)
...
if not isinstance(p_v, str):
    ...
    st.write(f"Marge brute sécurisée : {p_v - p_a} €")
else:
    st.error(p_v)
